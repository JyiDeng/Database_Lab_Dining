...
+ 进阶查询入口：
  ...
  + 查询当前用户在一周内不同点餐方式的销量：http://localhost:8014/admin/1/user-activity/weekly
  + 查询当前用户在一月内不同点餐方式的销量：http://localhost:8014/admin/1/user-activity/monthly
  + 查询当前用户在一年内不同点餐方式的销量：http://localhost:8014/admin/1/user-activity/yearly
  + 查询id=4的商户的忠实粉丝在一月内的消费分布：http://localhost:8014/merchant/4/loyalCustomer/monthly
  + 查询id=3的用户活跃度（点餐频率变化趋势、时间段）：http://localhost:8014/admin/1/user-activity/timeOfDay?userId=3
  + 查询当前用户活跃度（用户在一天不同时间段的活跃程度）：http://localhost:8014/admin/1/user-activity/timeOfDay?userId=3
  + 新增：查看id=5的商户在一个月内的用户评分趋势变化：http://localhost:8014/admin/1/ratingTrend/monthly?merchantId=5
...
+ 增加id为4的商户（如果已有这个id的用户会给出提示信息）：`http://localhost:8014/admin/1/addMerchant?id=4&name=Ramenya&mainDishes=Ramen&address=bentoRoadNo1`
+ 更新id为4的商户（如果不存在这个id的用户会给出提示信息）：`http://localhost:8014/admin/1/updateMerchant?id=4&name=Kidtucky&mainDishes=ChickenNuggets&address=FriedRoadNo2`
